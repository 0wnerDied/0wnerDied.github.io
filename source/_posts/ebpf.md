...
eBPF，一个源于 Linux 内核的革命性技术，它允许我们在内核这个特权环境中安全地运行沙盒程序。值得一提的是，BPF 最初是“伯克利数据包过滤器”的缩写，但如今它的能力已远超网络领域，因此 “BPF” 已成为一个独立术语，而其前身则被称为 **cBPF (classic BPF)**。

eBPF 程序并非直接在 CPU 上运行，而是在一个位于内核中的、高度安全和高效的**BPF 虚拟机 (VM)** 上执行。这个虚拟机就像一个沙盒，确保了 eBPF 代码不会搞砸内核。

为了让这个虚拟机能干活，我们需要给它一套指令，这就是 **BPF 指令集**。我们用 C 语言等高级语言写好的逻辑，通过编译器（如 LLVM/Clang）翻译成 BPF 指令集字节码，然后才能交给虚拟机执行。

但光有代码执行还不够，eBPF 程序经常需要和用户空间程序或者内核的其他部分交换数据，这时就需要用到 **BPF Maps**。它是一种高效的键值对存储，是 eBPF 程序与外界沟通的桥梁。

此外，为了让 eBPF 程序能与内核交互（比如获取当前进程 ID），内核提供了一组固定的**辅助函数 (Helper Functions)**。eBPF 程序只能调用这些预设的、安全的函数，从而在获得强大能力的同时，保证了内核的稳定。
...
- **指令集**: BPF 指令集是 BPF 虚拟机的“普通话”。所有指令都是固定长度的，这让处理更简单高效。
...
`LDD` (Class `0x00`, Mode `0x00`, Size `0x03`) 是一条非常特殊的指令，它使用 128 位的宽格式。它不仅能加载一个 64 位的常量，还能通过 `src_reg` 字段的不同取值，实现加载 Map 地址等高级功能，是连接 BPF 程序和外部资源的关键。
...
这解释了在 BPF C 代码中，我们看似直接引用的 Map 变量，在底层是如何通过 `ld_imm64` 指令被解析成实际内存地址的。
...
如果说 BPF 程序是处理数据的“工人”，那 **BPF Maps** 就是工人们存放工具、交换半成品的“仓库”。我们已经看到，`LDD` 指令是如何在底层将 Map 翻译成地址的。

BPF Maps 是一种通用的、存在于内核中的键值对存储。它有以下几个关键作用：

1.  **状态保持**: BPF 程序是无状态的，一次事件触发执行完所有信息就丢失了。Maps 可以在多次执行之间保存状态，比如用它来统计某个网络包出现的次数。
...
- `BPF_MAP_TYPE_HASH`: 高效的哈希表。
- `BPF_MAP_TYPE_ARRAY`: 高效的数组。
- `BPF_MAP_TYPE_PERF_EVENT_ARRAY`: 用于向用户空间发送性能事件数据。
- `BPF_MAP_TYPE_RINGBUF`: 高性能的环形缓冲区，是向用户空间传递数据的现代方式。

## 四、能力扩展：内核辅助函数

为了安全，BPF 虚拟机是一个与世隔绝的沙盒，它不能随意调用内核里的任意函数。那它如何与内核交互呢？答案是 **内核辅助函数 (Helper Functions)**。
...
- **获取上下文信息**: `bpf_get_current_pid_tgid()` (获取进程ID)、`bpf_ktime_get_ns()` (获取时间戳)
- **网络包处理**: `bpf_skb_store_bytes()` (修改网络包内容)
- **打印调试**: `bpf_printk()` (向内核日志打印信息)
...
- **Kprobes / Tracepoints**: 用于内核态函数追踪和性能分析，是可观测性工具（如 `bcc`、`bpftrace`）的基石。
...
我们通常使用 C 语言的一个子集来编写 BPF 程序，然后用 `LLVM/Clang` 将其编译成 BPF 字节码。
...
1.  **程序必须能终止**: 禁止无限循环，通过有向无环图 (DAG) 检查确保程序不会永远执行下去。
